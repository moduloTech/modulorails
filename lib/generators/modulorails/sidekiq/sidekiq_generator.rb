...
class Modulorails::SidekiqGenerator < Rails::Generators::Base
  ...
  def add_to_deploy_file
    add_to_deploy_file(Rails.root.join('config/deploy/production.yaml')) if File.exist?(Rails.root.join('config/deploy/production.yaml'))
    add_to_deploy_file(Rails.root.join('config/deploy/staging.yaml')) if File.exist?(Rails.root.join('config/deploy/staging.yaml'))
    add_to_deploy_file(Rails.root.join('config/deploy/review.yaml')) if File.exist?(Rails.root.join('config/deploy/review.yaml'))
  end
  ...
  def add_to_deploy_file(file_path)
    # Do nothing if sidekiq is already enabled
    return if File.read(file_path).match?(/^ {2}sidekiq:$/)
